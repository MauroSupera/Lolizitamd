...
exports.getPerfil = async(getPushName, getRecado, getMessagePrivado, getTotalMessageGroup, from, al_id, levelPatent) => {
try {
var getMessage = (await getRecado(al_id)).status;
} catch {
...
▢ ⚠︎ Info | Perfil 
▢ ╭═══⊷
▢ Nome: ${getPushName(al_id)}
▢ Level: ${levelPatent(from, al_id).nivel}
▢ Nivel: ${levelPatent(from, al_id).classification}
▢ Recado: ${getMessage}
...
exports.getPerfilSender = async(getPushName, getRecado, getMessagePrivado, getTotalMessageGroup, from, sender, levelPatent) => {
try {
var getMessage = (await getRecado(sender)).status;
} catch {
var getMessage = 'Recado privado';
};
...
▢ ⚠︎ Info | Perfil 
▢ ╭═══⊷
▢ Nome: ${getPushName(sender)}
▢ Level: ${levelPatent(from, sender).nivel}
▢ Nivel: ${levelPatent(from, sender).classification}
▢ Recado: ${getMessage}
...
exports.logo = () => {
const bufferMenu = [fs.readFileSync('./base de dados/fotos/photo1Menu.jpg'), fs.readFileSync('./base de dados/fotos/photo1Menu.jpg'), fs.readFileSync('./base de dados/fotos/photoMenu.jpg')]; 
return bufferMenu[Math.floor(Math.random() * bufferMenu.length)];
}